...
## Commercial impact: tying pipeline signals to Net New ARR

`commercial_impact.py` reads the reference run and estimates how this pipeline's actual measured outputs - correct Sales routing, expansion signals surfaced, retention risk caught - map to Net New ARR, the standard dollar-denominated SaaS revenue bridge: **New Logo ARR + Expansion ARR - Contraction ARR - Churned ARR**. This is deliberately the real, industry-standard bridge (the same one Salesforce reports internally as "NNAOV" - Net New Annual Order Value) rather than a made-up formula: New Logo, Expansion, Contraction, and Churn are all genuine dollar figures that add and subtract cleanly against each other.

**Why GRR and NRR are reported separately, not folded into the bridge.** Gross Revenue Retention and Net Revenue Retention are conventionally *percentages* - retention health ratios, not dollar line items - so this script reports them alongside the ARR bridge rather than inside it. An earlier draft of this feature tried to subtract "GRR ARR" directly from the bridge; that doesn't reconcile dimensionally (a ratio isn't a subtractable dollar amount) and was corrected before this shipped.

**Account tiering.** `classify_account_tier` buckets accounts into `self_serve` / `mid_market` / `enterprise` using `mid_market_arr_threshold` ($5,000) and `enterprise_arr_threshold` ($50,000) in `config.py` - presets, not universal constants, since every company sets these differently. Not wired into routing or confidence; account size alone doesn't change which team should own a message, only how `commercial_impact.py` weights the impact estimate.

**What's real vs. illustrative.** Real: which messages triggered which signal (Sales routing, `health_expansion_flag`, `account_health_is_risk`, formal close/cancel detection), and whether that signal could be tied to a known mock account with an actual `arr_usd` on file. Illustrative: the dollar amounts themselves - assumed deal sizes by team size, an assumed close rate, and assumed expansion/contraction rates, all in `config.py`, all placeholder assumptions a real deployment would replace with actual historical data.
...
- **The commercial-impact dollar figures are illustrative, and the sample is genuinely small.** 14 mock accounts is enough to demonstrate the calculation, not enough to make any real claim about revenue impact - see `commercial_impact.py`'s docstring and the negative-NNAOV note above.
